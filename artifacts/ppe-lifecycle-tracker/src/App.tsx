...
import {
  DEFAULT_RULES, SOURCE_ACTIVITIES, cloneSource, type Activity, type AssignmentStatus, type Employee,
...
function PpeProvider({ children }: { children: ReactNode }) {
  const [employees, setEmployees] = useState<Employee[]>(() => readStorage(STORAGE_KEY, cloneSource()));
  const [rules, setRules] = useState<RuleSet>(() => readStorage(RULES_KEY, DEFAULT_RULES));
  ...
  const updateAssignment = (employeeId: string, assignmentId: string, patch: Partial<PPEAssignment>) => {
    setEmployees((current) => current.map((employee) => {
      ...
      const old = employee.assignments.find((assignment) => assignment.id === assignmentId);
      ...
      return { ...employee, assignments: employee.assignments.map((item) => item.id === assignmentId ? next : item) };
    }));
...
function ruleName(value: string) { return value.split('::')[0]; }
...
function Dashboard() {
  ...
  const allAssignments = employees.flatMap((employee) => employee.assignments);
  ...
  const hotspots = subcenters.map((subcenter) => {
    ...
    const okay = subset.flatMap((employee) => employee.assignments).filter((assignment) => assignment.status === 'OK' && assignment.quantity > 0).length;
    return { subcenter, score: total ? Math.round(okay / total * 100) : 0, issues: subset.flatMap((employee) => employee.assignments).filter((assignment) => badStatuses.includes(assignment.status)).length };
...
function Register() {
  ...
  const filtered = employees.filter((employee) => {
    ...
    const employeeStatus = employee.assignments.some((item) => badStatuses.includes(item.status)) ? 'Needs attention' : 'Ready';
    ...
    <div className="card table-card">
     <div className="toolbar"><div className="search-wrap"><Search /><input className="input" type="search" placeholder="Search name, employee ID or mobile" value={query} onChange={(event) => setQuery(event.target.value)} data-testid="input-register-search" /></div><select className="select" value={skill} onChange={(event) => setSkill(event.target.value)} data-testid="select-register-skill"><option>All skills</option><option>Electrical</option><option>Electrical &amp; WAH</option></select><select className="select" value={subcenter} onChange={(event) => setSubcenter(event.target.value)} data-testid="select-register-subcenter"><option>All subcenters</option>{subcenters.map((item) => <option key={item}>{item}</option>)}</select><select className="select" value={status} onChange={(event) => setStatus(event.target.value)} data-testid="select-register-status">{statusOptions.map((item) => <option key={item}>{item}</option>)}</select><span className="filters-note">{filtered.length} of {employees.length} people</span></div>
      {filtered.length === 0 ? <EmptyState title="No people match these filters" copy="Try clearing one of the filters to see the full register." /> : <div className="table-scroll"><table className="data-table"><thead><tr><th>Employee</th><th>Skill / subcenter</th><th>Assigned PPE</th><th>Register health</th><th>Last inspection</th><th /></tr></thead><tbody>{filtered.map((employee) => {
        const latest = employee.assignments.map((item) => item.inspectionDate).sort().at(-1) || '';
        const hasIssue = employee.assignments.some((item) => badStatuses.includes(item.status));
...
function RegisterRow({ employee, expanded, onExpand, hasIssue, latest, rules, onUpdate }: { employee: Employee; expanded: boolean; onExpand: () => void; hasIssue: boolean; latest: string; rules: RuleSet; onUpdate: Store['updateAssignment'] }) {
  return <>
    <tr data-testid={`row-employee-${employee.id}`}><td><Link href={`/people/${employee.id}`} className="person-cell link-person" data-testid={`link-person-${employee.id}`}><span className="person-avatar">{initials(employee.name)}</span><span><span className="person-name">{employee.name}</span><span className="person-meta">{employee.id} · {employee.mobile}</span></span></Link></td><td><strong>{employee.skill}</strong><div className="person-meta">{employee.subcenter}</div></td><td><span className="mono">{employee.assignments.length} / {rules[employee.skill]?.length || 0}</span><div className="person-meta">items recorded</div></td><td><span className={`status ${hasIssue ? 'bad' : 'ok'}`}><span className="signal" />{hasIssue ? 'Needs attention' : 'Ready'}</span></td><td className="mono">{prettyDate(latest)}</td><td><button className="btn btn-soft btn-small" onClick={onExpand} data-testid={`button-expand-${employee.id}`}>{expanded ? <X size={14} /> : <Pencil size={14} />} {expanded ? 'Close' : 'Edit'}</button></td></tr>
     {expanded ? <tr><td colSpan={6} style={{ padding: 0, background: 'hsl(42 32% 96%)' }}><InlineEditor employee={employee} onUpdate={onUpdate} /></td></tr> : null}
  </>;
}

function InlineEditor({ employee, onUpdate }: { employee: Employee; onUpdate: Store['updateAssignment'] }) {
  return <div style={{ padding: '17px 20px 19px' }}><div style={{ display: 'grid', gap: 10 }}>{employee.assignments.map((assignment) => <div key={assignment.id} style={{ display: 'grid', gridTemplateColumns: '1.2fr .65fr .52fr .9fr .9fr .9fr 1fr auto', gap: 10, alignItems: 'end' }}><div><label className="field-label">{assignment.item}</label><div className="mono text-muted">{assignment.id || 'No PPE ID recorded'}</div></div><div><label className="field-label">Status</label><select className="select small-input" value={assignment.status} onChange={(event) => onUpdate(employee.id, assignment.id, { status: event.target.value as AssignmentStatus })} data-testid={`select-status-${assignment.id}`}><option>OK</option><option>Faulty</option><option>Missing</option><option>NOK</option><option>Due</option></select></div><div><label className="field-label">Qty</label><input className="input small-input" type="number" min="0" value={assignment.quantity} onChange={(event) => onUpdate(employee.id, assignment.id, { quantity: Math.max(0, Number(event.target.value) || 0) })} data-testid={`input-quantity-${assignment.id}`} /></div><div><label className="field-label">Purchased</label><input className="input small-input" type="date" value={assignment.purchaseDate} onChange={(event) => onUpdate(employee.id, assignment.id, { purchaseDate: event.target.value })} data-testid={`input-purchase-${assignment.id}`} /></div><div><label className="field-label">Issued</label><input className="input small-input" type="date" value={assignment.issueDate} onChange={(event) => onUpdate(employee.id, assignment.id, { issueDate: event.target.value })} data-testid={`input-issue-${assignment.id}`} /></div><div><label className="field-label">Inspection</label><input className="input small-input" type="date" value={assignment.inspectionDate} onChange={(event) => onUpdate(employee.id, assignment.id, { inspectionDate: event.target.value })} data-testid={`input-inspection-${assignment.id}`} /></div><div><label className="field-label">Reason</label><input className="input small-input" value={assignment.reason} placeholder="Add note" onChange={(event) => onUpdate(employee.id, assignment.id, { reason: event.target.value })} data-testid={`input-reason-${assignment.id}`} /></div><Link href={`/people/${employee.id}`} className="btn btn-soft btn-small" data-testid={`link-edit-person-${employee.id}`}>Details</Link></div>)}</div></div>;
}
...
function PersonDetail() {
  ...
  const { employees, updateAssignment, updateEmployee } = usePpe();
  ...
  return <section><div className="header-row"><div><Link href="/register" className="link-action" data-testid="link-back-register"><ArrowLeft size={13} /> Back to register</Link><h1 className="page-title" style={{ marginTop: 12 }}>{employee.name}</h1><p className="page-subtitle">{employee.id} · {employee.designation}</p></div><div className="actions"><a className="btn btn-soft" href={`tel:${employee.mobile}`} data-testid="link-call-employee"><Phone size={15} /> Call employee</a></div></div>
    <div className="detail-grid"><div className="card profile-card"><div className="profile-head"><span className="profile-avatar">{initials(employee.name)}</span><div><div className="profile-name">{employee.name}</div><div className="profile-role">{employee.skill}</div></div></div><dl className="info-list"><div className="info-item"><dt>Employee ID</dt><dd className="mono">{employee.id}</dd></div><div className="info-item"><dt>Mobile</dt><dd>{employee.mobile}</dd></div><div className="info-item"><dt>Designation</dt><dd>{employee.designation}</dd></div><div className="info-item"><dt>Subcenter</dt><dd>{employee.subcenter}</dd></div></dl><div style={{ marginTop: 22 }}><button className="btn btn-soft" onClick={() => updateEmployee(employee.id, { designation: employee.designation === 'Technician' ? 'Senior Technician' : 'Technician' })} data-testid="button-toggle-designation"><Pencil size={14} /> Toggle designation</button></div></div>
      <div className="section-stack"><div className="card panel"><div className="panel-heading"><div><h2 className="panel-title">Assigned PPE</h2><p className="panel-kicker">Edit inspection fields and status inline.</p></div><span className={`status ${employee.assignments.some((item) => badStatuses.includes(item.status)) ? 'bad' : 'ok'}`}>{employee.assignments.filter((item) => item.status === 'OK').length} / {employee.assignments.length} good</span></div><div className="assignment-list">{employee.assignments.map((assignment) => <AssignmentEditor key={assignment.id} employee={employee} assignment={assignment} onUpdate={updateAssignment} />)}</div></div><div className="card panel"><div className="panel-heading"><div><h2 className="panel-title">Inspection history</h2><p className="panel-kicker">Most recent recorded checks for {employee.name.split(' ')[0]}.</p></div><ActivityIcon size={17} className="text-muted" /></div><div className="action-list">{employee.assignments.slice(0, 4).map((assignment) => <div className="action-row" key={`history-${assignment.id}`}><span className={`signal ${assignment.status === 'OK' ? 'green' : 'red'}`} /><div className="action-main"><strong>{assignment.item}</strong><span>Inspected {prettyDate(assignment.inspectionDate)}{assignment.reason ? ` · ${assignment.reason}` : ''}</span></div><span className={`status ${statusClass(assignment.status)}`}>{assignment.status}</span></div>)}</div></div></div></div>
  </section>;
}
...
function AssignmentEditor({ employee, assignment, onUpdate }: { employee: Employee; assignment: PPEAssignment; onUpdate: Store['updateAssignment'] }) {
  return <div className="assignment-row"><div><div className="assignment-item">{assignment.item}</div><div className="assignment-id">{assignment.id || 'No PPE ID recorded'}</div></div><div><label className="field-label">Status</label><select className="select small-input" value={assignment.status} onChange={(event) => onUpdate(employee.id, assignment.id, { status: event.target.value as AssignmentStatus })} data-testid={`detail-status-${assignment.id}`}><option>OK</option><option>Faulty</option><option>Missing</option><option>NOK</option><option>Due</option></select></div><div><label className="field-label">Quantity</label><input className="input small-input" type="number" min="0" value={assignment.quantity} onChange={(event) => onUpdate(employee.id, assignment.id, { quantity: Math.max(0, Number(event.target.value) || 0) })} data-testid={`detail-quantity-${assignment.id}`} /></div><div><label className="field-label">Inspection</label><input className="input small-input" type="date" value={assignment.inspectionDate} onChange={(event) => onUpdate(employee.id, assignment.id, { inspectionDate: event.target.value })} data-testid={`detail-inspection-${assignment.id}`} /></div><div><label className="field-label">Purchase</label><input className="input small-input" type="date" value={assignment.purchaseDate} onChange={(event) => onUpdate(employee.id, assignment.id, { purchaseDate: event.target.value })} data-testid={`detail-purchase-${assignment.id}`} /></div><div><label className="field-label">Issue date</label><input className="input small-input" type="date" value={assignment.issueDate} onChange={(event) => onUpdate(employee.id, assignment.id, { issueDate: event.target.value })} data-testid={`detail-issue-${assignment.id}`} /></div><div><label className="field-label">Action</label><button className="btn btn-soft btn-small" onClick={() => onUpdate(employee.id, assignment.id, { status: 'OK', reason: '' })} data-testid={`button-clear-issue-${assignment.id}`}><Check size={13} /> Clear</button></div></div>;
}
...
function exportCsv(employees: Employee[]) {
  const rows = employees.flatMap((employee) => employee.assignments.map((item) => [employee.id, employee.name, employee.skill, employee.subcenter, item.item, item.id, item.quantity, item.purchaseDate, item.issueDate, item.inspectionDate, item.status, item.reason]));
  downloadFile([['Employee ID', 'Employee', 'Skill', 'Subcenter', 'PPE item', 'PPE ID', 'Quantity', 'Purchase date', 'Issue date', 'Inspection date', 'Status', 'Reason'], ...rows].map((row) => row.map((value) => `"${String(value).replaceAll('"', '""')}"`).join(',')).join('\n'), 'ppe-register.csv', 'text/csv');
}
